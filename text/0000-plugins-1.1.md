...
Right now an important use case in both serde and diesel is the ability to
configure custom-`#[derive]` modes. For example serde has configuration like
`#[serde(rename = "foo")]`, and diesel has attributes for defining relationships
beteen objects. Unfortunately, however, the compiler currently rejects custom
attributes!

To allow this form of customization, `rustc-macro` crates will be allowed to
inform the compiler that attributes within a namespace are allowed to exist.
This will be done through a crate attribute on the `rustc-macro` crate itself:

```rust
#![rustc_macro_attribute_namespace(serde)]
```

This informs the compiler that when this macro crate is loaded, all attributes
of the form `#[serde(..)]` are allowed. Each attribute will be ignored by the
compiler, and considered "used" to avoid unused attribute warnings. These are
then available for processing by custom-`#[derive]` implementations because the
token stream they receive will contain an item definition with attributes
applied.
