...
### Bitcasts and conversions

From/Into/FromBits/IntoBits are too controversial, but these operations are
required for this API to be useful: converting between vector types is an
extremely common operation, and also required for interfacing with the
architecture-specific types in `std::arch`
...
### Some operations with run-time indices are weird:
...
* `extract` / `replace` / `store` should probably require the index to be a compile-time constant.

* the `vec::new()` constructors should lower to an `llvm::ConstantVector` when
  their arguments are compile-time constants
  
* this removes the need for the unchecked mehtods  
...
things wrt load/stores aligned/unaligned.

(also related to bitcasts)

Endianness applies to the bytes in a lane and to the lanes within a vector.

The following asserts would pass/fail depending on endianness:
...
This does not necessarily need to be fixed in this RFC, but we should start
thinking about how to deal with fast math in Rust because fast-math can
significantly impact the "quality" of the generated vector code.

# Things we might want to remove

We could provide useful enough types, and implementation of the traits that are
part of the std library because users cannot provide these themselves. 

Anything else can be provided as a third party library.

### Reductions

Remove reductions from this iteration of the RFC - provide only the types,
enough methods to make them useful, and trait implementations that users
cannot provide themselves.

They are controversial because of overflow and NaNs (see below).
...
* vertical: that is, lane-wise. For example, `a + b` adds each lane of `a` with
  ...
  vectors). These operations are typically very fast - this is how vectors
  should be used.
  
* horizontal: that is, along a vector. Horizontal operations are unary, that is,
  they take one input vector. For example, `a.h_sum()` adds the elements of the
  vector together and `a.h_max()` returns the largest element in the vector.
  These operations are typically slower than vertical operations but in some
...
The following issues are part of the Unresolved Question sections and will need
to be resolved by future RFCs:

* half-float (`f16`) vectors (and half-floats in general) are part of the vendor
  APIs of ARM, AArch64, PowerPC64, RISC-V, MIPS, and others.
...
pub fn h_sum(self) -> element_type;

/// Horizontal product of the vector elements.
pub fn h_product(self) -> element_type;
...
assert_eq!(val, vec.h_sum()); // MAYBE FAILS
```
...
pub fn h_and(self) -> element_type;

/// Horizontal bitwise `or` of the vector elements.
pub fn h_or(self) -> element_type;

/// Horizontal bitwise `xor` of the vector elements.
pub fn h_xor(self) -> element_type;
...
pub fn h_max(self) -> element_type;

/// Value of the smallest element in the vector.
pub fn h_min(self) -> element_type;
...
operations exposed by this API in one go, like `(a + b).h_sum()` on a `f32x4`
vector. The zero-overhead requirement makes it a bug if Rust does not generate
optimal code for this situation.
...
Many vector intrinsics on many architectures (ARM,Power,MIPS,RISC-V) operates on
vectors of half-floats (`f16`) types. It is unclear what to do with these as
this point since the `f16` type is even less portable than the `{i,u}128` types.
...
* TODO: what's the problem with the AVX-512 masks .
